...
func run() error {
	...
	if hook := config.Env("BITRIX_WEBHOOK", ""); hook != "" {
		...
		// Владелец вебхука в логе — не украшение. Портал показывает реестру
		// переписку одного человека, и чужой вебхук проявляется не отказом, а
		// не тем списком чатов: полдня уходит на поиск поломки там, где её нет.
		if owner := svc.PortalOwner(ctx); owner != "" {
			log.Info("портал Bitrix24 подключён", "вебхук выдан на", owner)
		} else {
...
func logins(log *slog.Logger, addr string) (*auth.Auth, error) {
	...
	// Сообщение осталось от тех времён, когда пустой REESTR_SECRET означал
	// случайную подпись при каждом запуске; теперь ключ заводится в файл, и о
	// нём говорит signingKey. Пугать перелогином, которого не будет, незачем —
	// сказать стоит только про место, где ключа может не оказаться.
	if config.Env("REESTR_SECRET", "") == "" {
		log.Info("ключ подписи взят из " + keyFile +
			": в контейнере без тома задайте REESTR_SECRET, иначе вход слетит вместе с ним")
	}
	return a, nil
}
