...
type pullReport struct {
	...
	// Trouble отмечает, что до части чатов подтяжка не добралась. Браузер по
	// нему выбирает тревожный цвет сообщения: «перенесено 12 сообщений» зелёным
	// человек прочитает как «всё прочитано», а прочитано было не всё.
...
func newPullReport(res []service.PullResult) pullReport {
	...
	// О закрытом чате говорим всегда и называем его по имени: чинить это идут в
	// портал, и там нужно знать, в какую переписку добавить владельца вебхука.
	if len(closed) > 0 {
		out.Trouble = true
		out.Text += "; закрыт для реестра: " + strings.Join(closed, ", ")
	}
	return out
}
...
func newChatOptions(configured bool, owner string, list []bitrix.Chat) chatOptions {
	...
	// Имя владельца вебхука в надписи — не вежливость, а диагноз. Портал отдаёт
	// реестру чаты одного человека, и переписка контакт-центра у каждого
	// оператора своя: не найдя в списке своего чата, человек ищет поломку в
	// реестре, пока не увидит, что список вообще не его.
	//
	// Имя стоит после двоеточия именительным падежом, а не «чаты Александра
	// Волощука»: склонять фамилии сотрудников пришлось бы кодом, а ошибиться в
	// чужой фамилии хуже, чем обойтись без падежа. По той же причине после
	// двоеточий стоят и числа: «19 переписок из 41 чатов» требует двух разных
	// падежей сразу.
	whose := "список — чаты того, на кого выдан вебхук"
	if owner != "" {
		whose = "список — чаты одного человека: " + owner
	}
	...
	case !configured:
		opts.Note = "Bitrix24 не настроен: задача создастся без чата"
	case len(list) == 0:
		opts.Note = "портал не отдал ни одного чата. " + capitalize(whose)
	case len(client) == 0:
		opts.Note = "ни одной переписки с клиентом портал не показал. " +
			capitalize(whose) + ". Свою возьмите полем ниже, по адресу из портала"
	default:
		// Про «полем ниже» сказано здесь же и не случайно: список — чаты одного
		// человека, нужной переписки в нём может не быть, а открытую линию портал
		// отдаёт по адресу любому сотруднику.
		opts.Note = "переписок с клиентами: " + strconv.Itoa(len(client)) +
			", всего чатов: " + strconv.Itoa(len(list)) + ". " + capitalize(whose) +
			". Нет нужной — возьмите её полем ниже, по адресу из портала"
	}
	return opts
}

// capitalize поднимает первую букву. Нужна ровно надписям, которые собираются
// из кусков и потому попадают то в середину фразы, то в её начало.
