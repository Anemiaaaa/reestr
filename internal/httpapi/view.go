...
func newChatOptions(configured bool, owner string, list []bitrix.Chat) chatOptions {
	...
	case len(client) == 0:
		opts.Note = "среди чатов " + whose + " нет ни одной переписки с клиентом. " +
			"Свою — полем ниже: открытую линию портал отдаёт по номеру кому угодно"
	default:
		// «Чужую по номеру» — не оговорка, а самое полезное здесь предложение.
		// Список короткий не потому, что переписок мало, а потому что портал
		// показывает чаты одного человека; открытую же линию он отдаёт по номеру
		// любому сотруднику, и это единственный путь к своей переписке.
		opts.Note = ru.Count(len(client), "переписка", "переписки", "переписок") +
			" с клиентами из " + strconv.Itoa(len(list)) + " чатов " + whose + ". " +
			"Своей здесь нет — берите её полем ниже: открытую линию портал отдаёт по номеру кому угодно"
	}
	return opts
}
