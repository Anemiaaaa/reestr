...
//
// Так портал отвечает на закрытые для него переписки: личные и групповые чаты,
// в которых владелец вебхука не состоит. Открытые линии контакт-центра сюда не
// попадают — их портал отдаёт по номеру любому сотруднику, даже не участнику;
// проверено на живом портале и на карточке, и на сообщениях.
//
// Ошибка отдельная, потому что человеку тут нужен не «портал не смог», а точное
// «в этот чат вас не пускают»: исправляется это в портале, а не в реестре.
var ErrChatForbidden = errors.New("чат не виден владельцу вебхука")

// dialogRef вылавливает номер диалога из того, что человек вставил в поле.
//
// Три вида на входе: «28», «chat28» и целый адрес страницы портала вида
// .../online/?IM_DIALOG=chat28. Третий — главный: номер чата нигде в портале не
// написан цифрами, его берут из адресной строки, и требовать выковыривать
// оттуда «chat28» глазами значит требовать помнить наше соглашение.
var dialogRef = regexp.MustCompile(`chat(\d+)|^\s*(\d+)\s*$`)

// DialogID приводит написанное человеком к тому, что портал ждёт в DIALOG_ID.
//
// Пустая строка означает «не разобрал»: лучше сказать это словами, чем отправить
// в портал мусор и пересказать его ответ.
func DialogID(s string) string {
	m := dialogRef.FindStringSubmatch(s)
	if m == nil {
		return ""
	}
	if m[1] != "" {
...
// вебхука. Открытая линия при этом остаётся доступной для чтения — и карточка,
// и сообщения, — так что назвать её номером достаточно; это проверено на живом
// портале. Отказ в доступе бывает у чатов других видов.
func (c *Client) Chat(ctx context.Context, dialogID string) (Chat, error) {
	if strings.TrimSpace(dialogID) == "" {
		return Chat{}, &Error{Code: "DIALOG_ID_EMPTY", Description: "не указан чат"}
	}
	if dialogID = DialogID(dialogID); dialogID == "" {
		return Chat{}, &Error{
			Code:        "DIALOG_ID_BAD",
			Description: "не разобрал номер чата: нужен «28», «chat28» или адрес чата из портала",
		}
	}
