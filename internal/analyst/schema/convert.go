...
func (c *checker) check(field string, v Value) (domain.Value, bool) {
	...
	case "":
		// Происхождения нет вовсе: чаще всего это значит, что модель ответила
		// в этом поле голой строкой вместо значения с происхождением.
		c.reject(field, "значение без происхождения", v)
