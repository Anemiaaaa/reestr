...
// UnmarshalJSON принимает и объект, и голую строку.
//
// Строгий режим схемы не спасает: проверено на живом шлюзе — модель вернула в
// одном поле строку вместо значения с происхождением, и разбор всего ответа
// падал на этом одном поле. Восемьдесят секунд работы и оплаченный вызов
// пропадали целиком из-за одной ошибки в одном разделе.
//
// Строка становится значением без происхождения. Проверка его отбросит и
// скажет почему, но остальные разделы ответа при этом уцелеют: одно негодное
// поле не должно стоить всего разбора.
func (v *Value) UnmarshalJSON(b []byte) error {
	// Отдельный тип, чтобы не звать этот же метод рекурсивно.
	type plain Value
	...
	// Ни объект, ни строка: поле считаем незаполненным. Ронять разбор из-за
	// него незачем — проверка скажет, что поля не было.
	*v = Value{}
	return nil
}
...
type milestone struct {
	...
	// Weight — объём этапа относительно других. Ноль означает «не назван» и
	// считается за единицу: план, в котором объём не проставлен, обязан
	// считаться так же, как считался до появления этого поля.
