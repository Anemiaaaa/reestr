...
type PullResult struct {
	...
	// Problem — почему этот чат прочитать не вышло. Пусто, когда всё в порядке.
	//
	// Поле есть, потому что закрытый чат — это состояние одного чата, а не сбой
	// подтяжки. Прежде отказ в доступе прерывал всю задачу, и новые сообщения
	// открытой линии не приходили из-за чата задачи, в который человека не
	// добавили. Разбирается это в портале, и до тех пор остальное должно
	// работать.
	Problem string
}
...
func (s *Service) PullChats(ctx context.Context, taskID string) ([]PullResult, error) {
	...
	for _, link := range links {
		...
		if err != nil {
			// Закрытый чат — не сбой подтяжки, а его собственное состояние:
			// записываем и идём дальше. Всё прочее (портал не ответил, сеть
			// легла) прерывает, как прежде: молча дочитать половину переписки и
			// сказать «готово» — худший из возможных ответов.
			if !errors.Is(err, bitrix.ErrChatForbidden) {
				return nil, err
			}
			s.log.Warn("чат закрыт для владельца вебхука",
				"задача", taskID, "чат", link.DialogID, "название", link.Title)
			out = append(out, PullResult{
				DialogID:      link.DialogID,
				Title:         link.Title,
				LastMessageID: link.LastMessageID,
				Problem:       "в этот чат владельца вебхука не пускают — добавьте его в портале",
			})
			continue
		}
		out = append(out, res)
