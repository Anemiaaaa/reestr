...
async function pinChat() {
  ...
  const got = await ask("Прикрепить чат", [
    ...
    {
      name: "dialogId", label: "Или адрес чата из портала",
      hint: "вставьте адрес целиком — или 28, или chat28",
      note: "Откройте переписку в портале и скопируйте адрес из строки браузера. " +
        "Так берут свою переписку с клиентом, которой нет в списке выше, и чат " +
        "любого другого вида.",
    },
  ], null, true);
  if (!got) return;
...
async function addTask() {
  ...
  // filled — что мы подставили в форму сами, по полям. Нужен, чтобы отличать
  // своё от набранного человеком: своё перезаписываем при новом выборе, чужое
  // не трогаем.
  const filled = {};
  ...
  const taskField = {
    ...
    onChange(id, inputs) {
      ...
      // Своё перезаписываем, чужое — нет.
      //
      // Правило «заполнять только пустые поля» берегло набранное человеком, но
      // ломало обычный ход дела: выбрал задачу, увидел, что не та, выбрал
      // другую — и поля остались от первой, потому что пустыми уже не были.
      // Заказчик так и напоролся: искал нужную задачу, нашёл, выбрал, а в форме
      // осталась предыдущая.
      //
      // Поэтому помним, что подставили сами. Поле, в котором стоит наше
      // прошлое значение, — наше, его и меняем; поле, где стоит что-то другое,
      // человек правил руками, и трогать его нельзя.
      for (const name of FROM_PORTAL) {
        const input = inputs[name];
        if (!input) continue;
        if (input.value !== "" && input.value !== filled[name]) continue;
